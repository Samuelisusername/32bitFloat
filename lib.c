...
float_t fp_encode(float b) {
  ...
  if(fc.intval>>31) answer.sign = 1;
  else answer.sign = 0;

  answer.exponent = ((fc.intval << 1) >> (FRAC_BITS + 1));

  answer.mantissa = (fc.intval <<9)>>9;
  return answer;
}
...
float fp_decode(float_t a) {
  ...
  fc.floatval = 0;
  //adding the sign
  fc.intval = (a.sign <<31) | fc.intval;
  //adding the exponent
  fc.intval = (a.exponent << 23) | fc.intval;
  //adding the mantissa
  fc.intval = a.mantissa | fc.intval;
  return fc.floatval;
...
float_t fp_negate(float_t a) {
 ...
 /* float_t answer = {a.sign, a.exponent, a.mantissa};
  answer.sign= (a.sign)? 0:1;
